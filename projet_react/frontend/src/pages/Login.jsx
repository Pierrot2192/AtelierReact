...
const Login = () => {
  ...
  const loggedIn = async ({ username, password }) => {
    ...
    if(result.status === 200){
      ...
      toast.error('Il y a une erreur a la connexion');
    }
  };
  const formik = useFormik({
    ...
    validaitonSchema: Yup.object({
      username: Yup.string("Le nom d'user doit être une chaine de caractère")
      .min(3, "Le nom d'user doit faire au minimum 3 caractères")
      .required("Username est requis"),
      password: Yup.string("Le paswd doit être string")
      .min(3, "Le pasw doit faire mini 3charact")
      .required("Les paswd et requis"),
    }),
    onSubmit: (values) => {
      loggedIn(values);
    },
  ...
  <div className="h-screen flex">
    ...
    <div className="flex w-1/2 justify-center items-center bg-white">
      ...
      onSubmit={(e) => {
        ...
        <NavLink className="text-sm ml-2 hover:text-blue-500 cursor-pointer" to="/register">
          Créer un compte
        </NavLink>
      </form>
    </div>
